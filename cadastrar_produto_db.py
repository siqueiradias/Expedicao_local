...
class cadastrar_produto_db:
    @staticmethod    
    def buscar(cur_db):
        lista_resultado = []
        try:
            result = cur_db.execute(f"""SELECT * FROM tb_produto;""")
            for busca in result:
                lista_resultado.append(busca)
            return lista_resultado
        except Exception as e:
            print('Erro na Busca de Dados: ', e)
            r = ('000',"Produto Não Encontrado!", 0.0)
            return r

    @staticmethod
    def inserir(cursor, conexao, cod_prod, desc, peso_liq):
        try:
            cursor.execute(f"""INSERT INTO "main"."tb_produto"
            ("cod", "descricao", "peso")
            VALUES ('{cod_prod}', '{desc}', {peso_liq});""")
            conexao.commit()
            return (1)
            ...
            print("Erro ao incluir o produto: ", e)
            return (3, e)
